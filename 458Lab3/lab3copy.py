import urllib, urllib2, requests, struct, base64, os, hashlib, sys, sha1sample
...
def collisionSHA1():
    ...
    while True:
        print(x)
        result = hashlib.sha1(str(x)).hexdigest()
...
def lengthExtendSha1():
    ...
    addmsg = "Such as yours."
    for count in range(1, 65):
        newMsg = what + generatePadding(len(what) + count) + addmsg
        padMsg = addmsg + generatePadding(len(what) + count + len(generatePadding(len(what) + count) + addmsg))
        ...
        d = int(mac[24:32], 16)
        e = int(mac[32:40], 16)
        newMac = SHA1LE(padMsg, a, b, c, d, e)
